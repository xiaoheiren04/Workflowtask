var flexup = {
    ...
    checkInputNotEmpty: function() {
        ...
        if($this.type == "app") {
            ...
            if(region != undefined && region.length > 0 && num != "") {
                $("#btn-flexup-server").prop("disabled", false);
            }
            else {
                $("#btn-flexup-server").prop("disabled", true);
            }
        }
        else if($this.type == "ip" || $this.type == "name") {
            var host = $.trim($("#app-host").val());
            if(num != "") {
                $("#btn-flexup-server").prop("disabled", false);
            }
            else {
    ...
    confirmFlexup: function(event) {
        ...
        $("#flexup-cfm-modal").modal("show");
    },
    startFlexup: function(event) {
        var $this = this;
        $("#flexup-errmsg").hide();
        if($this.type == "app") {
            var payload = {
